...
private:
	...
	void set_raw(Uint32 raw)
	{
		...
		case 2: *static_cast<Uint16*>(target_) = static_cast<Uint16>(raw); break;
		case 3:
	...
	Uint32 get_raw() const
	{
		...
		case 3:
			if constexpr (SDL_BYTEORDER == SDL_BIG_ENDIAN)
			{
				return static_cast<Uint8*>(target_)[0] << 16 | static_cast<Uint8*>(target_)[1] << 8
					   | static_cast<Uint8*>(target_)[2];
			}
			else
			{
				return static_cast<Uint8*>(target_)[0] | static_cast<Uint8*>(target_)[1] << 8
