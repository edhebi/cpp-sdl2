...
namespace sdl
{
///\brief Represent an SDL window. Also contains accessor to any window related
///adjacent functionality like OpenGL/Vulkan helper functions
class Window
{
public:
	...
	/// Move assign operator. If this object represent a valid window, it will
	/// be destroyed before acquiring the window_ pointer from other \param other
	///Another sdl::Window object
	...
	/// 2D renderer factory. Permit to easilly create a 2D renderer. Hardware
	/// acceleration enabled by default \param flags Any flags needed to be
	///passed to SDL_CreateRenderer.
	...
	///\param filename path to a file you can use to set the window icon (may
	///require linking and activating SDL_Image)
	...
	/// Returns true if window is currently fullscreen (both real and "desktop
	/// mode"
	bool fullscreen()
	{
		return flags()
	...
	/// Get window manager info. Exact content of this structure is fully
	/// platform dependant.
...
#ifdef CPP_SDL2_VK_WINDOW
	/// Enumerate the requred extensions to create a VkSurfaceKHR on the current
	/// system
	...
	{
	public:
		/// Create a GlContext for the given window. You should use
		/// sdl::Window::gl_create_context() insdtead of this
