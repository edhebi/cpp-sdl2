...
namespace sdl
{
///\brief This class represent the "library entry point". Create and keep an
///instance of this object before creating any other sdl object (e.g.
///sdl::Window)
///
/// Create an sdl::Root object on the stack, and SDL is initialized.
/// When your instance of Root goes out of scope, SDL will be deinitialized for
/// you. This pattern (idiom) is called RAII. If you aren't familiar with it, I
/// suggest reading https://en.cppreference.com/w/cpp/language/raii
class [[nodiscard]]
Root{public:

		 /// Construct a root object. Will initialize the SDL with the privided
		 /// flags. Will thow an sdl::Exception if anything fails
			 Root(Uint32 flags){
