...
/// An SDL_Event and a sdl::Event object are "the same bits" in memory if they
/// hold infos about the same event. cpp-sdl2 convert the raw SDL_Event into an
/// sdl::Event object to add an object-oriented API around them
union Event
{
...
public:
	...
	*/
	...
	/// For type safety, we will use these scoped enum values instead of raw
	/// numbers like the C api
	...
	/// Wait until next event occur. This will stop the execution of your code
...
/// Pump the event loop from the OS enent system. only call this from the main
/// thread (or the thread taht initialized the video/window systems) This is only
/// usefull if you aren't polling or waiting for events
